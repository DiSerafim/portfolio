              ...
              {item.subMenu ? (
                <>
                  <div className="menu__item" onClick={toggleSubMenu}>
                    ...
                    <span className={`menu__text ${isOpen ? "" : "hidden"}`}>
                      {item.text}
                    </span>
                  ...
                  <ul
                    className={`submenu ${
                      isSubMenuOpen ? "submenu__open" : ""
                    }`}
                  >
                    {item.subMenu.map((subItem, subIndex) => (
                      <li className="nav__item" key={subIndex}>
                        <Link className="nav__link" to={subItem.to}>
                          <span className="icon" title={subItem.text}>
                            {subItem.icon}
                          </span>
                          <span
                            className={`menu__text ${isOpen ? "" : "hidden"}`}
                            title={subItem.text}
                          >
                            {subItem.text}
                          </span>
                ...
                <Link className="nav__link" to={item.to}>
                  <span className="icon" title={item.text}>
                    {item.icon}
                  </span>
                  <span
                    className={`menu__text ${isOpen ? "" : "hidden"}`}
